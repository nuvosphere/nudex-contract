...
contract AssetsTest is BaseTest {
    ...
    function test_Consolidate() public {
        ...
        // empty from address
        consolidateParams[0] = ConsolidateTaskParam("", TICKER, CHAIN_ID, amount, "hash1");
        vm.expectRevert("Invalid address");
        assetHandler.submitConsolidateTask(consolidateParams);

        // below minimum amount
        consolidateParams[0] = ConsolidateTaskParam(fromAddr, TICKER, CHAIN_ID, 0, "hash2");
        vm.expectRevert("Invalid amount");
        assetHandler.submitConsolidateTask(consolidateParams);

        // correct amount
        consolidateParams[0] = ConsolidateTaskParam(fromAddr, TICKER, CHAIN_ID, amount, "hash3");
        assetHandler.submitConsolidateTask(consolidateParams);
    ...
    function test_Transfer() public {
        ...
        // empty from address
        transferParams[0] = TransferParam("", toAddr, TICKER, CHAIN_ID, amount, "hash1");
        vm.expectRevert("Invalid address");
        assetHandler.submitTransferTask(transferParams);

        // empty from address
        transferParams[0] = TransferParam(fromAddr, "", TICKER, CHAIN_ID, amount,"hash2");
        vm.expectRevert("Invalid address");
        assetHandler.submitTransferTask(transferParams);

        // below minimum amount
        transferParams[0] = TransferParam(fromAddr, toAddr, TICKER, CHAIN_ID, 0, "hash3");
        vm.expectRevert("Invalid amount");
        assetHandler.submitTransferTask(transferParams);

        // correct amount
        transferParams[0] = TransferParam(fromAddr, toAddr, TICKER, CHAIN_ID, amount, "hash4");
        assetHandler.submitTransferTask(transferParams);
        taskOpts[0].extraData = TestHelper.getPaddedString(txHash);
        signature = _generateOptSignature(taskOpts, tssKey);
