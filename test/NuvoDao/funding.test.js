...
describe("Funding", function () {
    ...
    before(async function () {
        ...
        erc20Token = await ERC20.deploy("TestToken", "TT", ethers.utils.parseEther("1000000"));
    ...
    it("should handle funding proposals with native cryptocurrency", async function () {
        await dao.connect(member1).createProposal(
            ...
            ethers.utils.defaultAbiCoder.encode(
                ...
                [member1.address, ethers.utils.parseEther("10"), ethers.constants.AddressZero, "Development Fund"]
            ),
        ...
        expect(await ethers.provider.getBalance(member1.address)).to.equal(ethers.utils.parseEther("10"));
    });

    it("should handle funding proposals with ERC20 tokens", async function () {
        await erc20Token.transfer(dao.address, ethers.utils.parseEther("100"));
        ...
        await dao.connect(member1).createProposal(
            ...
            ethers.utils.defaultAbiCoder.encode(
                ["address", "uint256", "address", "string"],
                [member1.address, ethers.utils.parseEther("50"), erc20Token.address, "Marketing Fund"]
            ),
            { value: ethers.utils.parseEther("1") }
        );
        ...
        expect(await erc20Token.balanceOf(member1.address)).to.equal(ethers.utils.parseEther("50"));
    });
